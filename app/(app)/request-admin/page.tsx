...
export default async function RequestAdminPage() {
    ...
    return (
        <div className="max-w-sm space-y-4">
            <h1 className="text-2xl font-semibold text-gray-900 dark:text-gray-100">Request Admin Access</h1>
            {approved ? (
                <p className="text-sm text-green-700 dark:text-green-400">
                    Your request was approved. Please sign out and sign back in to activate admin access.
                </p>
            ) : pending ? (
                <p className="text-sm text-gray-600 dark:text-gray-400">Your request is pending review by an admin.</p>
            ) : (
                <>
