...
export async function PUT(request: Request, { params }: Params) {
    ...
    if (parentId) {
        ...
        if (wouldCreateCycle(id, parentId, parentMap)) {
            return NextResponse.json({ error: 'Cycle detected in team hierarchy' }, { status: 400 });
        }
    }
...
export async function DELETE(request: Request, { params }: Params) {
    ...
    const [subTeam] = await db.select({ id: teams.id }).from(teams).where(eq(teams.parentId, id)).limit(1);
